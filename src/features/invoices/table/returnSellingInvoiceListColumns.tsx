...
export function returnSellingInvoiceListColumns(
  dict: Dictionary,
...
): Column<ReturnSellingInvoiceWithoutProductsDto>[] {
  const indexColumn = rowIndexColumn<ReturnSellingInvoiceWithoutProductsDto>({
    pagination: rowIndexPagination,
    numberRowIndex,
  });

  const bodyColumns: Column<ReturnSellingInvoiceWithoutProductsDto>[] = [
    {
      ...
      accessor: function renderReturnId(row) {
        return (
          <Link
            href={`/manager/invoices/return-selling/${row.id}`}
