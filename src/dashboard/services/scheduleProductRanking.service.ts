...
export class ScheduleProductRankingService {
    ...
    // --- Public methods ---
    // Every 30 minutes: refresh daily top-100 Redis caches for all invoice types and today's price-trend day key.
    ...
    async calculateAndStoreProductRankingDaily(): Promise<void> {
        ...
        try {
            ...
            Logger.error('Error in: src/dashboard/services/scheduleProductRanking.service.ts');
            Logger.error(`Error details: ${error}`);
        }
    }

    // Every day at midnight: roll the previous calendar day into monthly aggregates, trim non-top-100 daily rows, refresh daily caches.
    ...
    // First day of month at midnight: roll the previous month into yearly aggregates, trim non-top-100 monthly rows, refresh monthly caches.
    @Cron('0 0 1 * *')
    ...
    // January 1st at 01:00: delete ranking rows older than 10 years.
    @Cron('0 1 1 1 *')
    ...
    // January 1st at midnight: trim non-top-100 yearly rows for the previous year and refresh yearly caches.
    @Cron('0 0 1 1 *')
