...
export class ReturnSellingInvoiceRepository {
    ...
    private async findOneReturnSellingInvoiceWithProductsAndUsernames(
        ...
        const queryBuilder = repo
            ...
            .andWhere('sellingInvoice.isDeleted = false');
    ...
    async confirmReturnSellingInvoice(
        ...
        // Re-check under the row lock: the selling invoice may have been soft deleted between the
        // service reading it and this transaction acquiring the lock.
        if (lockedSelling.isDeleted) return null;
    ...
    async getListOfReturnSellingInvoices(dto: GetListOfReturnSellingInvoiceRequestDto): Promise<{ data: ReturnSellingInvoiceEntity[]; total: number }> {
        ...
        // original selling invoice is soft deleted.
        qb.andWhere('sellInv.isDeleted = false');
        ...
        if (search && searchBy) {
            if (searchBy === 'returnInvoiceId') {
                ...
                const productSearchConditions = [`rsip.product_sku = :productSkuSearch`];
