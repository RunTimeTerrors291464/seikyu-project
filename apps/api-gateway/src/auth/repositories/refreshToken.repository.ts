...
export class RefreshTokenRepository {
    ...
    // Private properties.
    private readonly RefreshTokenPrefix: string = 'RT:'; //  api-gateway:auth:RT (refresh token).
    private readonly RefreshTokenIdPrefix: string = 'RTID:'; // api-gateway:auth:RTID (refresh token id). 
    private readonly RefreshTokenRateLimitPrefix: string = 'RTRL:'; // api-gateway:auth:RT_RL (refresh token rate limit).
    ...
    async storeRefreshToken(id: string, userId: string, refreshToken: string, expiresAt: Date): Promise<RefreshTokenEntity> {

        // Create the refresh token entity.
        const tokenEntity: RefreshTokenEntity = this.refreshTokenRepository.create({ id, userId, refreshToken, expiresAt });
        const savedToken: RefreshTokenEntity = await this.refreshTokenRepository.save(tokenEntity);
    ...
    async validateRefreshToken(refreshToken: string): Promise<RefreshTokenEntity | null> {
        ...
        const redisToken = await this.redisClient.hgetall(`${this.RefreshTokenPrefix}${refreshToken}`);
